...
class ESC_Gemini_API {
    ...
    private static function build_api_prompt( string $seed_name, ?string $variety = null, ?string $brand = null, ?string $sku_upc = null ) : string {
        ...
        $prompt .= "  \"image_url\": \"string|null - Suggest one high-quality image URL (HTTPS required) of the mature plant, fruit/flower, or seeds, suitable for display. IMPORTANT: Only provide URLs from public domains, Wikimedia Commons, or other reliable sources that allow direct image access. Avoid URLs from commercial seed catalogs (like Burpee, Baker Creek, Johnny's Seeds, Stark Bros) as they often block direct access. Verify the image is publicly accessible. Provide only the direct image URL string that ends with an image extension like .jpg, .jpeg, .png, or .gif.\",\n";
