...
class ESC_Image_Checker {
    ...
    private static function check_image_exists( $url ) {
        ...
        if ( strpos( $url, '/wp-content/uploads/' ) !== false ) {
            // For local network URLs, use wp_remote_head
            if ( strpos( $url, '192.168.1.128' ) !== false ) {
                ...
                // For local WordPress installs, try to convert URL to file path
                $upload_dir = wp_upload_dir();
                $file_path = str_replace( $upload_dir['baseurl'], $upload_dir['basedir'], $url );

                // If that doesn't work, try to extract the path and use the uploads directory
    ...
    private static function fix_image_url( $url ) {
        ...
        error_log('ESC Image Checker - Original URL: ' . $url);
        ...
        if ( strpos( $url, '/wp-content/uploads/' ) === 0 ) {
            ...
            error_log('ESC Image Checker - Added site URL to relative path: ' . $fixed_url);
            return $fixed_url;
        }
        ...
        if ( strpos( $url, '/wp-content/uploads/' ) !== false ) {
            ...
            error_log('ESC Image Checker - Replaced domain with site URL: ' . $fixed_url);
            return $fixed_url;
        }
        ...
        if ( strpos( $url, '//' ) === 0 ) {
            $fixed_url = 'http:' . $url;
            error_log('ESC Image Checker - Added protocol to protocol-relative URL: ' . $fixed_url);
            return $fixed_url;
        }
